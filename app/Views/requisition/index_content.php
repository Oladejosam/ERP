...
<?php endif; ?>
<?php if (!empty($_SESSION['delivery_note_id'])): ?><div class="alert alert-success"> <a href="/ERP/public/requisition/delivery-note?id=<?php echo (int)$_SESSION['delivery_note_id']; ?>" target="_blank">Print the delivery note</a></div><?php unset($_SESSION['delivery_note_id']); endif; ?>
                ...
                <div class="card-body">
                    ...
                    <?php foreach ($dispatchRequests as $dispatch): ?>
                        <div class="border-bottom py-3 d-flex flex-column flex-md-row justify-content-between align-items-md-center gap-3">
                            <div><strong><?php echo htmlspecialchars($dispatch['description'] ?? 'Inventory item'); ?></strong><div class="small text-muted"><?php echo htmlspecialchars($dispatch['title']); ?> · Requested by <?php echo htmlspecialchars($dispatch['requester_name']); ?> · Quantity: <?php echo number_format((float)$dispatch['quantity'], 2); ?> <?php echo htmlspecialchars($dispatch['unit'] ?? ''); ?></div><div class="small mt-1">Forwarded to: <strong><?php echo htmlspecialchars($dispatch['forwarded_to_name'] ?? 'Not forwarded'); ?></strong><?php if (($dispatch['handoff_decision'] ?? '') === 'flagged'): ?> · <span class="text-danger fw-semibold">Flagged</span><?php elseif (($dispatch['handoff_decision'] ?? '') === 'approved'): ?> · <span class="text-success fw-semibold">Recipient approved</span><?php endif; ?></div><div class="small mt-1">Free stock: <?php echo number_format((float)($dispatch['free_stock'] ?? 0), 2); ?> · Allocated stock: <?php echo number_format((float)($dispatch['allocated_stock'] ?? 0), 2); ?><?php if (!empty($dispatch['requisition_urgent'])): ?> · <span class="text-danger fw-semibold">Urgent</span><?php endif; ?></div></div>
                            <?php if (!empty($isStoreApprover)): ?><div class="d-flex flex-wrap gap-2"><form method="post" action="/ERP/public/requisition/dispatch/decide"><input type="hidden" name="dispatch_id" value="<?php echo (int)$dispatch['id']; ?>"><input type="hidden" name="stock_type" value="free"><input class="form-control form-control-sm" name="issued_to" maxlength="255" placeholder="Person, project, or site" required><button class="btn btn-success" type="submit" <?php echo (float)($dispatch['free_stock'] ?? 0) < (float)$dispatch['quantity'] ? 'disabled' : ''; ?> onclick="return confirm('Issue this request from free stock?');"><i class="bi bi-box-arrow-up me-1"></i>Issue Free Stock</button></form><form method="post" action="/ERP/public/requisition/dispatch/decide"><input type="hidden" name="dispatch_id" value="<?php echo (int)$dispatch['id']; ?>"><input type="hidden" name="stock_type" value="allocated"><input class="form-control form-control-sm" name="issued_to" maxlength="255" placeholder="Person, project, or site" required><label class="small d-flex align-items-center gap-1"><input type="checkbox" name="urgent" value="1"> Urgent</label><button class="btn btn-outline-warning" type="submit" <?php echo (float)($dispatch['allocated_stock'] ?? 0) < (float)$dispatch['quantity'] ? 'disabled' : ''; ?> onclick="return confirm('Issue this request from allocated stock?');"><i class="bi bi-box-arrow-up me-1"></i>Issue Allocated Stock</button></form></div><?php endif; ?>
                        </div>
                    <?php endforeach; ?>
                </div>
            </div>
